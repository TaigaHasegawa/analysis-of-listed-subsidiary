...
def main():
    ...
    df = pl_df.merge(attribute_df, how="left", left_on=["ticker_code"], right_on=["株式コード（固有名コード）"])
    ...
    file_name = "processed_financial.csv"
    df.to_csv(os.path.join(PROCESSED_DIR, file_name), encoding='utf_8_sig', index=False)
    file_name = "processed_segment.csv"
    segment_df.to_csv(os.path.join(PROCESSED_DIR, file_name), encoding='utf_8_sig', index=False)
